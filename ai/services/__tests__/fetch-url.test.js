...
describe("content negotiation", () => {
	...
	it("routes text/plain bodies that start with several comments to the HTML reader", async () => {
		const page =
			"\uFEFF<!-- license -->\n<!-- generated by tool -->\n<!DOCTYPE html><html><head><title>T</title><script>var x = 1;</script></head>" +
			`<body><nav>menu</nav><main><p>${"Paragraph. ".repeat(30)}</p></main></body></html>`;
		const routes = {
			"https://example.com/page.txt": response(page, { contentType: "text/plain" }),
		};
		const { result } = await run({ url: "https://example.com/page.txt" }, { routes });
		...
		const broken = await run(
			...
			{
				routes: {
					"https://example.com/broken.txt": response("<!-- never closed <html><body>x", {
						contentType: "text/plain",
					}),
				},
			}
		);
		expect(broken.result.source).toBe("text");
	});
	...
	it("sniffs XHTML (application/xhtml+xml) as XML: the meta charset has no authority", async () => {
		...
		for (const url of ["https://example.com/utf8.xhtml", "https://example.com/latin.xhtml"]) {
			const { result } = await run({ url }, { routes });
			expect(result.ok).toBe(true);
			// XHTML is XML: handed over verbatim (the HTML parser would misread CDATA
			// sections and self-closing elements), correctly decoded
			expect(result.source).toBe("text");
			expect(result.contentType).toBe("application/xhtml+xml");
			expect(result.content).toContain("<p>Café du marché</p>");
		}
	});
