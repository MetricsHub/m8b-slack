...
async function _ensureConnected(server) {
  ...
  if (server.client) {
    try {
      ...
      console.log(`[MCP] Connection to ${server.server_label} appears dead (${e.message || e}), reconnecting...`);
      try {
...
function _indexHostsFromList(server, hostsData) {
  ...
  try {
    ...
    console.log(`[MCP] Indexed ${indexedCount} hosts from ${server.server_label}. Total host keys: ${state.hosts.size}`);
  } catch (e) {
...
export async function initializeMcpRegistry(logger) {
  ...
  if (state.servers.length === 0) {
    ...
    if (url && token) {
      ...
      state.servers.push({ server_url: url, server_label: 'm8b-agent-01', token, tools: new Map(), client: null });
    }
  }
  ...
  for (const server of state.servers) {
    try {
      ...
      _log(logger, 'info', 'Tools discovered', { server: server.server_label, count: tools.length });
      ...
      if (server.tools.has('ListHosts')) {
        try {
          const res = await server.client.callTool({ name: 'ListHosts', arguments: {} }, undefined, { timeout: 60000 });
...
export async function executeMcpFunctionCall(name, args, logger) {
    ...
    try {
      ...
      for (const [key, entry] of state.hosts.entries()) {
        ...
        if (rx.test(key) || (hn && rx.test(hn))) {
          out[key] = { server_label: entry.server_label, attributes: entry.attributes, protocols: entry.protocols };
        }
      }
  ...
  for (const [, { server, hosts: hs }] of buckets) {
    ...
    const connected = await _ensureConnected(server);
    if (!connected) {
      results.push({ server_label: server.server_label, ok: false, error: 'Failed to connect to MCP server' });
      continue;
    }
    ...
    try {
      ...
      console.log(`[MCP] Calling tool '${name}' on ${server.server_label} with args:`, JSON.stringify(callArgs));
      const startTime = Date.now();
      const res = await server.client.callTool({ name, arguments: callArgs }, undefined, { timeout: 120000 });
      console.log(`[MCP] Tool '${name}' on ${server.server_label} completed in ${Date.now() - startTime}ms`);
