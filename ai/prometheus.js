  ...
  step: "60s"`,
    parameters: {
      ...
      properties: {
        ...
        start: {
          type: 'string',
          description: 'Start time for range queries. RFC3339 format or Unix timestamp. Required for range queries.',
        },
        end: {
          type: 'string',
          description: 'End time for range queries. RFC3339 format or Unix timestamp. Required for range queries.',
        },
        step: {
          type: 'string',
          description: 'Query resolution step width for range queries (e.g., "60s", "5m", "1h"). Defaults to "60s" if not specified.',
        },
      },
      required: ['query'],
...
export async function executePromQLQuery(args, logger) {
  ...
  if (!prometheusUrl) {
    return { ok: false, error: 'Prometheus URL not configured (M8B_PROMETHEUS_URL environment variable not set)' };
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return { ok: false, error: `Prometheus query failed: ${response.status} ${response.statusText}`, details: errorText };
    }
    ...
    if (data.status !== 'success') {
      return { ok: false, error: `Prometheus query error: ${data.error || 'Unknown error'}`, errorType: data.errorType };
    }
